...
// src/addr.js
var IS_DARWIN = typeof Process !== "undefined" && Process.platform === "darwin";
function formatIPv6(groups) {
  const mapped = groups[0] === 0 && groups[1] === 0 && groups[2] === 0 && groups[3] === 0 && groups[4] === 0 && groups[5] === 65535;
  if (mapped) {
...
function parseSockaddr(addr, isDarwin = IS_DARWIN) {
  try {
    if (addr === null || addr === void 0 || addr.isNull()) return null;
...
function signatureFrames(frames) {
...
// src/index.js
var IS_DARWIN2 = Process.platform === "darwin";
var LIBC_CANDIDATES = {
  linux: ["libc.so", "libc.so.6"],
  darwin: ["libSystem.B.dylib"]
}[Process.platform] || ["libc.so"];
var MAX_EVENTS = 500;
var MAX_STACKS_PER_DEST = 8;
var MAX_WALKS_PER_DEST = 48;
var COUNTS_INTERVAL_MS = 2e3;
var POINTER_SIZE = Process.pointerSize;
var SOCKADDR_ON_ENTER = { connect: 1, sendto: 4 };
var SOCKADDR_ON_LEAVE = { recvfrom: 4 };
var MSGHDR_ON_ENTER = ["sendmsg"];
var MSGHDR_ON_LEAVE = ["recvmsg"];
var PEER_HOOKS = ["send", "recv"];
var FD_HOOKS = ["read", "write", "__read_chk", "__write_chk"];
var CLOSE_HOOKS = ["close"];
var DUP_HOOKS = ["dup2", "dup3"];
var sockFds = {};
var counts = {};
var sigSeen = {};
var walks = {};
...
try {
  bridgeReady = frida_java_bridge_default.available === true;
} catch (e) {
  bridgeError = "frida-java-bridge threw: " + (e && e.message ? e.message : e);
}
var runtimePresent = hasAndroidRuntime() || bridgeReady;
...
function javaStack() {
  ...
  try {
    if (frida_java_bridge_default.vm === null || frida_java_bridge_default.vm.tryGetEnv() === null) return [null, "native-thread"];
    if (ExceptionClass === null) ExceptionClass = frida_java_bridge_default.use("java.lang.Exception");
    ...
    if (stackErrorSample === null) {
      ...
      send({
        type: "socket_trace_warning",
        dump: "socket_trace_meta.json",
        data_type: "json",
        data: { warning: "java stack walk threw: " + stackErrorSample + " \u2014 affected records are marked stack-error, not native" }
      });
    }
...
function parseMsghdr(ptr_) {
  try {
    ...
    if (ptr_.add(POINTER_SIZE).readU32() === 0) return null;
    return parseSockaddr(name, IS_DARWIN2);
  } catch (e) {
    return null;
  }
}
...
function sendCounts() {
  send({
    ...
    data: {
      ...
      stack_error_sample: stackErrorSample
    }
  });
}
function emit(fd, operation, peer, threadId) {
  ...
  const signature = frames ? "s" + hashFrames(frames.map((f) => f.str)) : `t${threadId}|${source}`;
...
function handlerFor(name) {
  if (Object.prototype.hasOwnProperty.call(SOCKADDR_ON_ENTER, name)) {
    ...
    return {
      onEnter(args) {
        ...
        if (fd < 0) return;
        let peer = parseSockaddr(args[argIndex], IS_DARWIN2);
  ...
  if (Object.prototype.hasOwnProperty.call(SOCKADDR_ON_LEAVE, name)) {
    ...
    return {
      ...
      onLeave(retval) {
        if (this.dtFd === void 0 || this.dtFd < 0 || retval.toInt32() < 0) return;
        let peer = parseSockaddr(this.dtAddr, IS_DARWIN2);
  ...
  if (PEER_HOOKS.includes(name)) {
    return {
      onEnter(args) {
        ...
        if (fd < 0) return;
        let peer = sockFds[fd];
        if (peer === void 0) peer = learn(fd, peerOf(fd));
  ...
  if (DUP_HOOKS.includes(name)) {
    return {
      onEnter(args) {
        ...
        if (newFd >= 0) delete sockFds[newFd];
      },
      onLeave(retval) {
        const result = retval.toInt32();
        if (result < 0 || this.dtOld === void 0) return;
        const peer = sockFds[this.dtOld];
        if (peer !== void 0) sockFds[result] = peer;
      }
    };
  }
  return null;
...
function installHooks() {
  const wanted = Object.keys(SOCKADDR_ON_ENTER).concat(Object.keys(SOCKADDR_ON_LEAVE)).concat(
    MSGHDR_ON_ENTER,
    MSGHDR_ON_LEAVE,
    PEER_HOOKS,
    FD_HOOKS,
  ...
  if (runtimePresent && !bridgeReady) {
    send({
      ...
      data_type: "json",
      data: {
        error: "java call-stack attribution unavailable: " + (bridgeError || "bridge not initialised") + " \u2014 records will have no attribution",
